...
%\documentclass[oneside,11pt,letterpaper]{memoir} % Font size
\documentclass[oneside,13pt,letterpaper]{memoir} % Font size

\input{structure.tex} % Include the file that specifies the document structure and layout

\title{Bash for Beginners} % Book title
\author{J. Evan Noynaert} % Author
\newcommand{\edition}{2020-06-15 Edition} % Book edition
...
\ThisCenterWallPaper{1.12}{cover.png} % Add the background image, the first argument is the scaling - adjust this as necessary so the image fits the entire page

\begin{tikzpicture}[remember picture,overlay]
\node [rectangle, rounded corners, fill=green, opacity=0.75, anchor=south west, minimum width=8cm, minimum height=6cm] (box) at (-0.5,-10) (box){}; % White rectangle - "minimum width/height" adjust the width and height of the box; "(-0.5,-10)" adjusts the position on the page
...
\node[anchor=west, color01, xshift=-1.5cm, yshift=-2cm, text width=7.0cm, font=\sffamily\bfseries\scshape\Large] at (box.north){\thetitle}; % "Text width" adjusts the wrapping width, "xshift/yshift" adjust the position relative to the white rectangle
